    ...
    success (res) {
      ...
      console.log(latitude, longitude);
        ...
        addressName_fun(latitude + "," + longitude,(address)=>{
          console.log("获取用户地理位置", latitude, longitude, speed, accuracy, address);
          wx.setStorageSync("startAddress", { title: address, address: address, location: { lat: latitude, lng: longitude } });
          callback({ startAddress: { title: address, address: address, location: { lat: latitude, lng: longitude } }})
        })
...
function getPolyline(callback) {
  ...
  const froms = startAddress.location.lat+","+startAddress.location.lng+"&to="+
    endAddress.location.lat + "," + endAddress.location.lng
  wx.request({
    url: `https://apis.map.qq.com/ws/direction/v1/bicycling/?from=${froms}&key=Z6RBZ-6H4CU-AV6V5-2OTIO-Q2CLZ-VMF5T`,
    header: {
